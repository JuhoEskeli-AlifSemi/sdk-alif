...
/* Tracks how many mono samples have been received in the current chunk. */
static volatile int g_samples_received;

static int mix_mono_output(int16_t *in, size_t in_size, int16_t *out, size_t out_size)
{
    size_t num_samples = in_size / 2;

    if (out_size < (int)num_samples) {
        LOG_ERR("mix_mono_output param failure %d - %d", (int)out_size, (int)num_samples);
        return -EINVAL;
    }
...
static int audio_handle_rx(void)
{
    ...
    while (offset < user_len) {
        ...
        int stereo_samples = size / SAMPLE_SIZE;
        rc = mix_mono_output(static_cast<int16_t *>(buffer), stereo_samples,
        ...
        offset += stereo_samples / 2;
        g_samples_received = offset;
...
static int32_t pdm_mode_set(int sampling_rate)
{
    ...
    case 8000:  return PDM_MODE_STANDARD_VOICE_512_CLK_FRQ;
    case 16000: return PDM_MODE_HIGH_QUALITY_1024_CLK_FRQ;
    case 32000: return PDM_MODE_WIDE_BANDWIDTH_AUDIO_1536_CLK_FRQ;
    case 48000: return PDM_MODE_FULL_BANDWIDTH_AUDIO_3071_CLK_FRQ;
    case 96000: return PDM_MODE_ULTRASOUND_4800_CLOCK_FRQ;
    default:    return -1;
    }
}
...
int audio_init(int sampling_rate)
{
    ...
    k_thread_create(&audio_thread, audio_thread_stack,
                    ...
                    audio_worker_thread, NULL, NULL, NULL,
